...
## v0.5 用户态 + 文件系统（下一步）
- [ ] 用户态进程：TSS + ring3 + syscall gate + ELF 加载器
- [ ] 内存：按进程页表 + copy-on-write 雏形
- [ ] 文件系统：FAT12/16 读取（软盘镜像直接可用）
- [ ] 设备：ATA/PATA 磁盘驱动、RTC 实时钟
- [ ] 桌面：鼠标驱动（PS/2）→ 窗口拖拽、聚焦点击
- 验收：可加载 `userspace/init.bin` 作为首个用户进程；`ls` 列出 FAT 镜像内容
